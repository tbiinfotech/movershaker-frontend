...
const SECRET_KEY_FOR_MESSAGE_ENCRYPTION =
  '762UDJ1189782090KSCJKKsgghsj7w828yisuhs'
...
export const getOtherPrivateChatMember = async (
  chat: Chat,
  excludedID: string,
) => {
...
export const formatDate = (time: any) => {
  ...
  if (typeof time.toDate === 'function')
    return dayjs(time.toDate()).format('DD/MM HH:mm')
...
export const encryptMessage = (message: string) => {
  ...
  const encryptedMessage = CryptoJS.AES.encrypt(
    message,
    SECRET_KEY_FOR_MESSAGE_ENCRYPTION,
  ).toString()
