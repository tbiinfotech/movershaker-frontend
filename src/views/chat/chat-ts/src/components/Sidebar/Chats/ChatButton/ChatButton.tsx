...
import { cutText, formatDate, getOtherPrivateChatMember, decryptMessage } from '../../../../utils';
import { Avatar, Box, Button, Typography } from '@mui/material';
// import useStyles from './styles';
import { StyledButton, StyledLink, ActiveItem, ChatAvatar, ChatTitle, RoundedButton, RecentMessage, RecentMessageUnread } from './styles';
import { useParams } from 'react-router-dom';
import { setCurrentChat } from '../../../../actions';
import { useAuthState } from 'react-firebase-hooks/auth';
import { auth, db } from '../../../../firebase';
import User from '../../../../../../../../types/User';
import { collection, onSnapshot, query, orderBy, limit, updateDoc, doc, deleteDoc, addDoc, getDoc } from 'firebase/firestore';
...
const ChatButton = ({ chat }: Props) => {
  ...
  useEffect(() => {
    ...
    // console.log('current chat changes ################################3', currentChat);
  }, [currentChat]);

  useEffect(() => {
    const fetchMember = async () => {
      const otherMember = await getOtherPrivateChatMember(chat, loggedInUser.user._id);
  ...
  const subscribeMessages = () => {
    ...
    const unsubscribe = onSnapshot(messagesRef, (snapshot) => {
      ...
      let countUnreadMessages = messages.filter(
        (m: any) => m.sentBy.uid != loggedInUser.user._id && !(m.isRead || []).includes(loggedInUser.user._id)
      ).length;
  ...
  return (
    // <Button className={`${classes.button} ${isActive ? classes.active : ''}`} onClick={() => dispatch(setCurrentChat(chat))} fullWidth>
    <StyledButton onClick={() => dispatch(setCurrentChat(chat))} fullWidth $active={isActive}>
      <Box display="flex" alignItems="center" width="100%" p={1}>
        ...
        <Box display="flex" flexDirection="column" ml={2} width="100%">
          <Box display="flex" justifyContent="space-between">
            {/* <Box display="flex" flexDirection="column" ml={2} width="100%" className={classes.chat_title}> */}
            <ChatTitle display="flex" flexDirection="column" ml={2} width="100%">
              <Typography align="left" sx={{ color: 'rgba(0,0,0,0.87)' }}>
                {chat.type === 'private'
              ...
              {countOfUnreadMessages == 0 ? (
                <RecentMessageUnread variant="caption" color="textSecondary">
                  {chat.recentMessage ? `${cutText(decryptMessage(chat.recentMessage.text), 12)}` : ``}
                </RecentMessageUnread>
              ) : (
                <RecentMessage variant="caption" color="textSecondary">
                  {chat.recentMessage ? `${cutText(decryptMessage(chat.recentMessage.text), 12)}` : ``}
                </RecentMessage>
              )}
              {/* <Typography
                variant="caption"
                color="textSecondary"
                className={countOfUnreadMessages !== 0 ? classes.recentMessageUnread : classes.recentMessage}
              > */}
              {/* {chat.recentMessage ? `${cutText(decryptMessage(chat.recentMessage.text), 12)}` : ``} */}
              {/* </Typography> */}
            </ChatTitle>
            <Box display="flex" flexDirection="column">
              ...
              {countOfUnreadMessages !== 0 && (
                // <Typography variant="caption" color="textSecondary" className={classes.roundedButton}>
                <RoundedButton
                  // variant="caption"
                  color="textSecondary"
                >
