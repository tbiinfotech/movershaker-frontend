    ...
    const fetchOtherMember = async () => {
      ...
      if (selectedChat) {
        if (selectedChat.type === 'private') {
          const otherMemberData = await getOtherPrivateChatMember(
            selectedChat,
          ...
          if (otherMemberData?.status === 'online') {
            ...
            if(otherMemberData?.lastSeen){
              setStatus(
                `Last seen ${new Date(otherMemberData.lastSeen * 1000).toLocaleTimeString()}`,
              )
            }else{
              setStatus('offline')
            }
      ...
      {chat ? (
        <Box display="flex" justifyContent="space-between" alignItems="center">
          <Box display="flex" alignItems="center">
            <Avatar
              src={
                chat.type === 'private' ? otherMember?.photoURL : chat?.photoURL
              }
          ...
          {chat.type === 'private' ? (
            ...
            <PublicMenu
              chat={chat}
              isOwner={chat.createdBy === loggedInUser.user._id}
              infoPage={false}
            />
          )}
        </Box>
