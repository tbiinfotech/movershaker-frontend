...
const UserSearch = ({ onItemClick, onCancel, avoidIdList }: Props) => {
  ...
  const filterResults = (word: string, users: User[]) => {
    ...
    return users.filter(
      (otherUser) =>
        otherUser.username.match(regex) &&
        otherUser.uid !== user?.uid &&
        !avoidIdList?.includes(otherUser.uid),
    )
  }
  ...
  const eligibleUsers = filterResults(input, users).filter((user) => {
    ...
    const isMember = chats
      .filter((chat) => chat.type === 'private')
      .some((chat) => chat.members.some((member) => member.uid === user.uid))
    return !isMember
  })
  ...
  return (
    <Box p={2} width="300px">
      ...
      {usersList.length > 0 ? (
        ...
        <List className={classes.list}>User Not found</List>
      )}
